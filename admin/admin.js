...
async function loadProducts() { const box = qs('#productsAdmin'); box.innerHTML = '<p class="text-sm opacity-60">تحميل...</p>'; try { const resp = await fetchJSON(API + '/products', { headers: authHeaders() }); box.innerHTML = ''; resp.data.forEach(p => { const card = ce('div', 'admin-card p-4 text-sm space-y-2'); card.innerHTML = `<div class='font-semibold'>${p.name}</div><div>${p.price} ج.س</div><input type='number' step='0.01' value='${p.price}' data-id='${p.id}' class='admin-input text-xs' /><div class='flex gap-2 mt-2'><button class='updateP admin-btn admin-btn-ghost text-xs' data-id='${p.id}'>تحديث</button><button class='delP admin-btn admin-btn-danger text-xs' data-id='${p.id}'>حذف</button></div>`; box.appendChild(card); }); } catch (e) { box.innerHTML = '<p class="text-sm text-red-300">خطأ تحميل المنتجات</p>'; } }
...
// Categories
async function loadCategories() { const box = qs('#categoriesAdmin'); if (!box) return; box.innerHTML = '<p class="text-sm opacity-60">تحميل...</p>'; try { const resp = await fetchJSON(API + '/categories', { headers: authHeaders() }); box.innerHTML = ''; resp.data.forEach(c => { const row = ce('div', 'admin-card p-3 text-sm flex justify-between items-center'); row.innerHTML = `<div class='flex items-center gap-3'>${c.image ? `<img src='${c.image}' class='w-10 h-10 rounded-lg object-cover border border-white/10' />` : `<div class='w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center text-xs opacity-30'>N/A</div>`}<span class='font-semibold'>${c.name}</span></div><div class='flex gap-2'><button class='editCat admin-btn admin-btn-ghost text-xs px-2 py-1' data-id='${c.id}'>تعديل</button><button class='delCat admin-btn admin-btn-danger text-xs px-2 py-1' data-id='${c.id}'>حذف</button></div>`; box.appendChild(row); }); } catch (e) { box.innerHTML = '<p class="text-sm text-red-300">خطأ تحميل التصنيفات</p>'; } }
async function addCategoryFormHandler(e) {
  ...
  try {
    ...
    alert('خطأ إضافة تصنيف');
  }
}
async function deleteCategory(id) {
  if (!confirm('حذف التصنيف؟')) return;
  const btn = document.querySelector(`.delCat[data-id="${id}"]`);
  const row = btn?.closest('.admin-card');
  if (row) row.style.opacity = '0.5';

  try {
    const res = await fetchJSON(API + '/categories/' + id, { method: 'DELETE', headers: authHeaders() });
    if (res.success) {
      if (row) row.remove();
...
async function deleteBanner(id) {
  ...
  const btn = document.querySelector(`.delBan[data-id="${id}"]`);
  const row = btn?.closest('.admin-card');
  if (row) row.style.opacity = '0.5';

  try {
    const res = await fetchJSON(API + '/banners/' + id, { method: 'DELETE', headers: authHeaders() });
    if (res.success) {
      if (row) row.remove();
      adminToast('تم الحذف بنجاح');
    }
...
async function loadTestimonials() {
  ...
  try {
    ...
    resp.data.forEach(t => {
      ...
      const row = ce('div', 'admin-card text-sm space-y-1');
...
async function deleteTestimonial(id) {
  ...
  const btn = document.querySelector(`.delTest[data-id="${id}"]`);
  const row = btn?.closest('.admin-card');
  if (row) row.style.opacity = '0.5';

  try {
    const res = await fetchJSON(API + '/testimonials/' + id, { method: 'DELETE', headers: authHeaders() });
    if (res.success) {
      if (row) row.remove();
...
async function loadOrders() {
  ...
  try {
    ...
    box.innerHTML = '';
    resp.data.forEach(o => {
      const row = ce('div', 'order-admin-card-admin bg-white/5 border border-white/10 rounded-2xl shadow-lg overflow-hidden transition hover:shadow-2xl relative');
...
function adminToast(msg) {
  let t = qs('#admin_toast');
  if (!t) {
    t = ce('div', 'fixed bottom-8 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-xl border border-white/20 text-white px-6 py-3 rounded-2xl shadow-2xl text-sm z-[100] transition-all duration-300 opacity-0');
    t.id = 'admin_toast';
...
function bind() {
  ...
  const cf = qs('#categoryForm'); cf && cf.addEventListener('submit', addCategoryFormHandler);
  ...
  qs('#ordersAdmin').addEventListener('click', async e => {
    ...
    if (e.target.classList.contains('deliver-order')) {
      const id = row.getAttribute('data-id');
      try {
        await fetchJSON(API + `/orders/${id}/status`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ status: 'Delivered' }) });
        row.remove(); notify('تم وسم الطلب كمُسَلَّم وإخفاؤه');
      } catch (err) { alert('تعذر تحديث حالة الطلب'); }
      return;
    }
  ...
  qs('#ordersAdmin').addEventListener('change', async e => {
    ...
    try {
      await fetchJSON(API + `/orders/${id}/status`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ status: value }) });
...
async function pollMetrics() {
  try {
    const r = await fetchJSON(API + '/metrics', { headers: authHeaders() }); const m = r.data || {};
...
async function init() {
  setupTabs(); bind(); await Promise.all([
